...
import { setForgotPin } from "../apiCalls/apiCalls";
...
const PinSetupCardReset = () => {
  ...
  const handleConfirm = async () => {
    ...
    if (pinValue === confirmPinValue && pinValue.length === 6) {
      try {
        console.log(data);
        await setForgotPin(data);
        navigate("/resetpinsuccess");
      } catch (error: any) {
    ...
    <div className="flex items-center justify-center w-full">
      <div className="bg-white p-2 w-[400px]">
        ...
        <div className="space-y-3">
          {/* Setup PIN */}
          <div>
            <label className="font-bold text-xs">Setup 6 DIGIT PIN</label>
          ...
          {/* Confirm PIN */}
          <div>
            <label className="font-bold text-xs">Confirm PIN</label>
            <div className="flex items-center justify-around gap-1 relative">
              {renderPinInputs(
                confirmPin,
                setConfirmPin,
              ...
              <button
                ...
                className="relative"
              >
                {showConfirmPin ? <EyeClosed size={24} /> : <Eye size={24} />}
              </button>
            </div>
          </div>
        </div>
...
export default PinSetupCardReset;
